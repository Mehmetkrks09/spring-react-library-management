...
public class KitapService {
	...
	public List<KitapDTO>getByCategoryId(@PathVariable Long categoryId){
		
		return kitapRepository.findByCategoryId(categoryId).stream()
				.map(kitapMapper:: entityToDto)
					.collect(Collectors.toList());
	}
	
	
	public List<KitapDTO> tumKitaplarıGetir() {
		List<Kitap> kitaplar = kitapRepository.findAll();

		return kitaplar.stream().map(kitapMapper::entityToDto).collect(Collectors.toList());
	}

	public KitapDTO kitapGetir(Long id) {
	...
	@Transactional
	public KitapDTO kitapKaydet(KitapDTO dto) {
	    ...
	    if (dto.getCategoryId() != null) {
	        Category category = categoryRepository.findById(dto.getCategoryId())
	            .orElseThrow(() -> new CategoryNotFoundException("ID "+dto.getCategoryId()+" Olan Kategori Bulunamadı"));
	...
	@Transactional
	public KitapDTO kitapGuncelle(Long id, KitapDTO kitapDto) {
